...
def apply(connection):
    ...
    if "computer_sessions" in existing:
        cs_cols = {
            row[1] for row in connection.exec_driver_sql("PRAGMA table_info(computer_sessions)")
        }
    ...
    if "browser_sessions" in existing:
        bs_cols = {
            row[1] for row in connection.exec_driver_sql("PRAGMA table_info(browser_sessions)")
        }
        ...
        if "active_tab_count" not in bs_cols:
            connection.exec_driver_sql(
                "ALTER TABLE browser_sessions ADD COLUMN active_tab_count "
    ...
    if "agent_connector_access" in existing:
        aca_cols = {
            row[1]
            for row in connection.exec_driver_sql("PRAGMA table_info(agent_connector_access)")
        }
