...
from hufiagents.contracts import AgentConnectorAccess, ConnectorRegistration, Risk, Routine, now
...
def test_routine_is_persistent_lifecycle_audited_and_dispatched(tmp_path):
    ...
    service = RoutineService(store, lambda template: submitted.append(template) or type("Mission", (), {"id": "m-1"})())
    routine = service.create(Routine(owner_agent_id="builder", mission_template={"outcome": "check"},
                                     schedule="every day at 08:00", timezone="Europe/Berlin"))
    ...
    with store.transaction() as tx:
        ...
        assert {event.event_type for event in tx.audit.list()} >= {
            "routine_created", "routine_updated", "routine_dispatched", "routine_archived"
        }
    store.close()
...
def test_workspace_and_browser_are_owned_durable_and_do_not_start_processes(tmp_path):
    ...
    assert service.workspace_path(workspace.id) == (tmp_path / "controlled-workspaces" / "builder-work")
...
def test_connector_grants_are_least_privilege_and_never_persist_secret(tmp_path):
    ...
    access = registry.grant(AgentConnectorAccess(
        agent_id="builder", connector_id=github.id, capabilities=["repo.read"], modes=["read"], risk_ceiling=Risk.R1
    ))
    assert access.status == "active"
    with pytest.raises(PermissionError):
        registry.grant(AgentConnectorAccess(
            agent_id="builder", connector_id=github.id, capabilities=["pull_request.draft.create"],
            modes=["write"], risk_ceiling=Risk.R1
        ))
