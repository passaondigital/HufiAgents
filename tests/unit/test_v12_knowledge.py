...
def test_scoped_memory_isolation_and_secret_sanitization():
    ...
    a = service.create_memory(ScopedMemory(scope_type="agent", scope_id="a", category="fact",
                                           summary="token test", content="token=ghp_FAKE123"))
    service.create_memory(ScopedMemory(scope_type="agent", scope_id="b", category="fact",
                                       summary="other", content="visible"))
...
def test_context_is_bounded_and_records_compaction():
    store = Store("sqlite:///:memory:")
    service = KnowledgeService(store)
    service.create_memory(ScopedMemory(scope_type="project", scope_id="p", category="fact",
                                       summary="important", content="x" * 500))
    result = service.assemble_context("important", scopes=[("project", "p")],
                                      max_context_chars=100, max_context_tokens_estimate=30)
