...
- [ ] Vom Modell intern verweigerte/abgelehnte Aufträge werden **nicht** identisch zu echten Erfolgen dargestellt (heute: gleiche grüne "Fertig"-Pille — `PRODUCT-FLOW-FINDINGS.md` MUSS Punkt 2, **kritisch**)
...
- [ ] Freigaben sind über den normalen Chat-Pfad tatsächlich erreichbar (heute: `risk_ceiling` im Chat hart auf R1 verdrahtet, strukturell unerreichbar — `PRODUCT-FLOW-FINDINGS.md` MUSS Punkt 3, **kritisch**)
- [ ] Nach Freigabe wird das Mission-Polling nachweislich fortgesetzt, ohne dass der Nutzer neu laden muss (im Code vorgesehen, aber mangels erreichbarer Freigabe nicht live durchspielbar)
...
- [ ] Eine im Chat formulierte Routine ("Mach das jeden Montag …") wird als solche erkannt, statt stillschweigend als Einmal-Auftrag ausgeführt (**kritischster Einzelbefund des Flow-Audits**, `PRODUCT-FLOW-FINDINGS.md` MUSS Punkt 1)
- [ ] Der Nutzer wird bei einer erkannten Routine-Absicht zur bereits vorhandenen ehrlichen Botschaft geleitet, statt eine irreführende Fake-Mission zu erhalten
- [ ] Echte wiederkehrende Ausführung ist möglich (blockiert auf Routinen-Modul, PR #12)
- [ ] Routine-Liste zeigt nächsten Lauf, aktiv/pausiert, letztes Ergebnis (Zielbild aus der Produktvision, noch nicht baubar)
