...
> **Update (Branch `claude/v1-1-release-product-fixes`):** alle drei MUSS-Punkte unten sind
> behoben und live gegen den integrierten PR-#14-Stand verifiziert (echter Qwen-Router, echte
> `POST /routines`/`POST /agents`/`POST /approvals`). Details, Code-Zeigern und
> Verifikationsnachweise: `docs/product/V1X-RELEASE-FIXES.md`. Dieser Abschnitt bleibt als
> historischer Audit-Befund unverändert stehen.
